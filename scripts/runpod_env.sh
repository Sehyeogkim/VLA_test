...
# Miniforge lives on the persistent volume so it survives Pod replacement.
export MINIFORGE_ROOT="${MINIFORGE_ROOT:-${VLA_WORKSPACE}/environments/miniforge3}"
if [[ -d "${MINIFORGE_ROOT}/bin" ]]; then
